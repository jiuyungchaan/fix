...
class FixTrader : public CTsSecurityFtdcTraderSpi {
 ...
 private:
  CTsSecurityFtdcTraderApi *trader_api_;
  std::string user_id_;
...
class FixQuery : public CTsSecurityFtdcQuerySpi {
 public:
  FixQuery();
 ...
 private:
  CTsSecurityFtdcQueryApi *trader_api_;
  std::string user_id_;
  std::string password_;
  std::string front_addr_;
  OrderPool order_pool_;
...
void FixTrader::Init() {
  ...
  trader_api_ = CTsSecurityFtdcTraderApi::CreateFtdcTraderApi();
...
void FixTrader::OnRtnTrade(CSecurityFtdcTradeField *pTrade) {
  cout << "OnRtnTrade- " << pTrade->Price << " " << pTrade->Volume << endl;
}
...
void FixQuery::Init() {
  // string config_file = user_id_ + ".cfg";
  trader_api_ = CTsSecurityFtdcQueryApi::CreateFtdcQueryApi();
  trader_api_->RegisterSpi(this);
  trader_api_->RegisterFront(const_cast<char*>(front_addr_.c_str()));
  trader_api_->Init();
}
...
int main(int argc, char **argv) {
  ...
  string front_addr = "tcp://112.74.17.91:5354";
  while (arg_pos < argc) {
